...
exports.register = async (req, res) => {
  try {
    ...
    if (date_of_birth) {
      ...
      if (age < 18) {
        return res
          .json({ message: "Người đăng ký phải từ 18 tuổi trở lên." });
      }
    }
...
exports.loginWithGoogle = async (req, res) => {
  try {
    ...
    if (!email || !fullName) {
      return res
        .status(400)
        .json({ message: "Thiếu thông tin tài khoản Google." });
    }

    // 1. Kiểm tra user
    const [rows] = await sql.query("SELECT * FROM User WHERE Email = ?", [
      email,
    ]);
    let user;
    ...
    if (rows.length === 0) {
      ...
      const [newRows] = await sql.query("SELECT * FROM User WHERE Email = ?", [
        email,
      ]);
      user = newRows[0];
    } else {
